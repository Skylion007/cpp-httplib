...
ClientImpl::open_stream(const std::string &method, const std::string &path,
  ...
  // Prepare headers (defaults + required) via helper
...
inline bool ClientImpl::write_request(Stream &strm, Request &req,
  ...
  // Prepare common defaults that are shared between streaming and non-
  // streaming paths. For requests with a body and no explicit Content-Type,
  // preserve the historical default of "text/plain" by passing it as the
  // content-type hint.
 ...
 * C++20 Streaming API
 *
 * This section provides C++20 coroutine-based streaming functionality.
 * Requires C++20 or later. For C++11/14/17, use open_stream() directly.
 *
 * Features:
 *   - Generator<T>: Coroutine-based lazy sequence generator
 *   - stream::Result: Wrapper for streaming HTTP responses
 *   - stream::Get/Post/Put/Patch/Delete/Head/Options: Streaming HTTP methods
 *
 * Usage:
